...
def default_dcor_top_latent_sae_training() -> TrainConfig:
    return TrainConfig(lr=5e-4)

def default_dcor_top_latent_sae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0, independence_loss="DcorTopLatent", independence_weight=1.0)

def default_dcor_top_latent_sae_model() -> ModelConfig:
    return ModelConfig(use_matryoshka=False, activation="TopKReLU_64")

@dataclasses.dataclass
class DcorTopLatentSAEConfig:
    """
    ...
    """
    training: TrainConfig = field(default_factory=default_dcor_top_latent_sae_training)
    loss: LossConfig = field(default_factory=default_dcor_top_latent_sae_loss)
    model: ModelConfig = field(default_factory=default_dcor_top_latent_sae_model)


def default_dcor_random_latent_sae_training() -> TrainConfig:
    return TrainConfig(lr=5e-4)

def default_dcor_random_latent_sae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0, independence_loss="DcorRandomLatent", independence_weight=1.0)

def default_dcor_random_latent_sae_model() -> ModelConfig:
    return ModelConfig(use_matryoshka=False, activation="TopKReLU_64")

@dataclasses.dataclass
class DcorRandomLatentSAEConfig:
    training: TrainConfig = field(default_factory=default_dcor_random_latent_sae_training)
...
def default_dcor_top_recon_sae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0, independence_loss="DcorTopRecon", independence_weight=1.0)

def default_dcor_top_recon_sae_model() -> ModelConfig:
    return ModelConfig(use_matryoshka=False, activation="TopKReLU_64")

@dataclasses.dataclass
class DcorTopReconSAEConfig:
    training: TrainConfig = field(default_factory=default_dcor_top_recon_sae_training)
    loss: LossConfig = field(default_factory=default_dcor_top_recon_sae_loss)
    model: ModelConfig = field(default_factory=default_dcor_top_recon_sae_model)

def default_dcor_random_recon_sae_training() -> TrainConfig:
    return TrainConfig(lr=5e-4)

def default_dcor_random_recon_sae_loss() -> LossConfig:
    return LossConfig(sparse_weight=0.0, independence_loss="DcorRandomRecon", independence_weight=1.0)

def default_dcor_random_recon_sae_model() -> ModelConfig:
    return ModelConfig(use_matryoshka=False, activation="TopKReLU_64")

@dataclasses.dataclass
class DcorRandomReconSAEConfig:
...
def get_config(model_name: str):
    ...
    elif model_name == "DcorTopLatentSAE":
