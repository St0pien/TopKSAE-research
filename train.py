...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Sparse Autoencoder (SAE) models"
    )
    parser.add_argument(
        "-dt",
        "--dataset_train",
...
def eval(model, eval_loader, loss_fn, device, cfg):
    ...
    logger.info(
        f"  Cosine Similarity Sparse: {diagonal_cs_sparse_sum / len(eval_loader):.4f}"
    )
    logger.info(
        f"  MAE Distance Sparse: {mae_distance_sparse_sum / len(eval_loader):.4f}"
    )
    logger.info(
        f"  Cosine Similarity All: {diagonal_cs_all_sum / len(eval_loader):.4f}"
    )
...
def main(args):
    ...
    logger.info(
        f"Training dataset length: {len(train_ds)}, Evaluation dataset length: {len(eval_ds)}, Embedding size: {train_ds.vector_size}"
    )
    logger.info(
        f"Training dataset mean center: {train_ds.mean.mean()}, Scaling factor: {train_ds.scaling_factor} with target norm {train_ds.target_norm}"
    )
    logger.info(
        f"Evaluation dataset mean center: {eval_ds.mean.mean()}, Scaling factor: {eval_ds.scaling_factor} with target norm {eval_ds.target_norm}"
    )
    assert (
        train_ds.vector_size == eval_ds.vector_size
    ), "Training and evaluation datasets must have the same embedding size"
    if args.dataset_second_modality is not None:
        ...
        logger.info(
            f"Second modality dataset mean center: {eval_ds_second.mean.mean()}, Scaling factor: {eval_ds_second.scaling_factor} with target norm {eval_ds_second.target_norm}"
        )
        assert (
            train_ds.vector_size == eval_ds_second.vector_size
        ), "Training and second modality datasets must have the same embedding size"
    ...
    logger.info(
        f"Expansion factor: {args.expansion_factor}, Latent dimensions: {cfg.model.n_latents}"
    )
    ...
    if cfg.model.use_matryoshka:
        ...
        if isinstance(cfg.model.nesting_list, int):
            logger.info(
                f"Generating nesting list from {cfg.model.nesting_list} to {max_nesting}"
            )
        ...
        if cfg.model.relative_importance == "RW":
            ...
            cfg.model.relative_importance = list(
                reversed(list(range(1, len(cfg.model.nesting_list) + 1)))
            )
        ...
        logger.info(
            f"Using Matryoshka with nesting list: {cfg.model.nesting_list} and weighting function: {cfg.model.relative_importance}"
        )
    else:
    ...
    logger.info(
        f"Calculating bias initialization with median: {cfg.training.bias_init_median}"
    )
    bias_init = 0.0
    if cfg.training.bias_init_median:
        # Use geometric median of a subset of data points for robustness
        bias_init = geometric_median(
            train_ds, device=device, max_number=len(train_ds) // 10
        )
    ...
    if not cfg.training.mean_center:
        mean_input = calculate_vector_mean(
            train_ds, num_workers=cfg.training.num_workers
        )
    ...
    if cfg.training.scheduler == 1:
        ...
        scheduler = LinearDecayLR(
            optimizer, cfg.training.epochs, decay_time=cfg.training.decay_time
        )
    ...
    for epoch in range(cfg.training.epochs):
        ...
        for step, embeddings in enumerate(tqdm(train_loader, desc="Training")):
            ...
            recons_sparse, recons_all, embeddings = (
                recons_sparse.detach(),
                recons_all.detach(),
                embeddings.detach(),
            )
            ...
            diagonal_cs_sparse, mae_distance_sparse = calculate_similarity_metrics(
                recons_sparse, embeddings
            )
            diagonal_cs_all, mae_distance_all = calculate_similarity_metrics(
                recons_all, embeddings
            )
            ...
            if global_step % cfg.training.print_freq == 0:
                logger.info(
                    f"Epoch: {epoch+1}, Step: {step}, Loss: {loss.item():.6f}, Recon Loss: {recon_loss.item():.6f}, Sparse Loss: {sparse_loss.item():.6f} Independence Loss: {independence_loss.item():.6f}"
                )
                logger.info(
                    f"FVU Sparse: {fvu_score_sparse:.4f}, FVU All: {fvu_score_all:.4f}"
                )
                logger.info(
                    f"CKNNA Sparse: {cknna_score_sparse:.4f}, CKNNA All: {cknna_score_all:.4f}"
                )
                logger.info(
                    f"Cosine Similarity Sparse: {diagonal_cs_sparse:.4f}, MAE Distance Sparse: {mae_distance_sparse:.4f}"
                )
                logger.info(
                    f"Cosine Similarity All: {diagonal_cs_all:.4f}, MAE Distance All: {mae_distance_all:.4f}"
                )
                logger.info(
                    f"Sparsity Sparse: {sparsity_sparse:.4f}, Sparsity All: {sparsity_all:.4f}"
                )
                logger.info(
                    f"Orthogonal Decoder Loss: {od:.6f}, Representation norm {repr_norm:.4f} and max {repr_max:.2f}"
                )
        ...
        logger.info(
            f"Epoch: {epoch+1}, Learning Rate: {lr_rate:.6f}, Loss: {loss.item():.6f}, Recon Loss: {recon_loss.item():.6f}, Sparse Loss: {sparse_loss.item():.6f}, Independence Loss: {independence_loss.item():.6f}, Dead neurons: {numb_of_dead_neurons} {alpha_log}"
        )
